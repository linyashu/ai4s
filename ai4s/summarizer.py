...
def _select_candidates(cfg: dict, payload: list[dict], limit: int) -> list[dict]:
    ...
    prompt = (
        ...
        "【来源一致性】以上收录/排除规则对全部来源（arXiv、期刊、中文媒体、HF）"
        "同等适用，不因来源是新闻而放宽。新闻类条目的判定要点：\n"
        "- 报道的是\"AI 技术/模型/人才/产业\"本身 → 收录\n"
        "- 报道的是\"AI 在生命科学（病毒/药物/医学）、气候、能源等自然科学领域的"
        "应用成果\" → 即使来自科技媒体，也一律排除\n"
        "新闻反例（不要选）：AI 设计新病毒、AI 飓风/气候预测、AI 医学助手/健康应用、"
        "AI 蛋白质设计、AI 材料研发。\n\n"
        "条目列表（JSON）：\n"
        + json.dumps(payload, ensure_ascii=False, indent=2)
    )
...
def _refine(cfg: dict, shortlist: list[dict]) -> Digest:
    ...
    prompt = (
        ...
        "此规则对全部来源同等适用，包括新闻类——即使新闻来自科技媒体，"
        "只要其核心是\"AI 在生命科学（病毒/药物/医学）、气候、能源等自然科学领域的应用成果\"，"
        "也一律剔除，不因来源是新闻而放宽\n"

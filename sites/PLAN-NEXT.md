...
| 里程碑 | 内容 | 状态 |
|---|---|---|
| M1 安全与正确性 | 阶段一全部 + 提交 | ✅ 完成（2026-08-13） |
| M2 工程化落地 | 测试 + CI + 依赖清理 + 时区 + 管线稳健 | ✅ 完成（2026-08-13，45 项测试） |
| M2.5 生产部署 | VPS Docker 部署 + 定时 ingest | ✅ 完成（2026-08-13，http://107.182.191.63/ai4s/） |
| M3 体验 1.0 | 真实投票 + 时间窗口 + LLM 日报 + 首页速览 | ✅ 完成（2026-08-13，已上线验证） |
| M4 内容升级 | 告警/成本仪表盘/数据保留 | ⬜ |
| M5 规模化 | 数据保留 + SQL 分页 + 搜索 | ⬜ |

## 部署记录（2026-08-13）

- VPS `107.182.191.63`（Debian 13 + Docker 29），代码位于 `/opt/ai4s-sites`
- Nginx `/ai4s/` 反代容器 3000 端口；`ai4s.bak` 已移出 sites-enabled（重复 server 块）
- 已知坑：① Nginx 对 `/ai4s` 自动 301 到 `/ai4s/`，Next.js 构建期需注入 `BASE_PATH`（compose build args）否则 308 循环；② Next.js 路由 300s maxDuration，完整 ingest 需用 `docker exec tsx` 方式（crontab 每 6 小时）；③ curl 访问 API 需 `-L` 跟随 308
- 生产修复：processWithLLM JSON 截断容错（VPS 实测 failed 0/279）
